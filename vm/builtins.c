...
static value builtin_objget( value o, value f ) {
	...
	val_check(f,int);
	return val_field(o,val_to_field(f));
}
...
static value builtin_objset( value o, value f, value v ) {
	if( !val_is_object(o) )
		return val_null; // keep dot-access semantics
	val_check(f,int);
	alloc_field(o,val_to_field(f),v);
	return v;
}
...
static value builtin_objcall( value o, value f, value args ) {
	...
	val_check(f,int);
	val_check(args,array);
	return val_ocallN(o,val_to_field(f),val_array_ptr(args),val_array_size(args));
}

static value builtin_objfield( value o, value f ) {
	return alloc_bool( val_is_object(o) && val_is_int(f) && otable_find(((vobject*)o)->table, val_to_field(f)) != NULL );
}

static value builtin_objremove( value o, value f ) {
	val_check(o,object);
	val_check(f,int);
	return alloc_bool( otable_remove(((vobject*)o)->table,val_to_field(f)) );
}
...
static value builtin_field( value f ) {
	val_check(f,int);
	return val_field_name(val_to_field(f));
}
