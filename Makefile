...
dep:
	@echo '# auto generated by "make dep"' > ${DEPFILE}
	${CC} ${CFLAGS} -MM ${subst .o,.c, ${VM_OBJECTS}} | sed -e 's|^\(.*:\)|vm/\1|' >> ${DEPFILE}
	${CC} ${CFLAGS} -MM ${subst .o,.c, ${STD_OBJECTS}} | sed -e 's|^\(.*:\)|libs/std/\1|' >> ${DEPFILE}
	${CC} ${CFLAGS} -MM ${subst .o,.c, ${LIBNEKO_OBJECTS}} | sed -e 's|^\(.*:\)|vm/\1|' >> ${DEPFILE}
...
.PHONY: all libneko libs neko std compiler clean doc test dep
